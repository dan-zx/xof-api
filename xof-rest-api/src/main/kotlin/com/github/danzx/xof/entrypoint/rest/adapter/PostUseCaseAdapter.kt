...
class PostUseCaseAdapter {
    ...
    fun getAll(filter: PostsFilter, pagination: Pagination) =
        getPostsUseCase(PostsLoaderCommand(filter, pagination, sortBy { +"created" }))
        .toPageResponse()

    fun replaceTitle(id: Long, title: String) = replacePostTitleUseCase(ReplacePostTitleCommand(id, title))

    fun replaceContent(id: Long, content: String) = replacePostContentUseCase(ReplacePostContentCommand(id, content))
