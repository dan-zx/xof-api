...
class ReplacePostContentEndpointTest : PostRestControllerBaseTest() {

    @Test
    fun `should replace post content return 200 (Ok) when payload is valid, id is $ge 1 and post exists`() {
        val expected = TEST_POST.copy(content = VALID_REQUEST.value!!)
        every { replacePostContentUseCase(any()) } returns expected
    ...
    @ParameterizedTest
    @NullAndEmptySource
    fun `should replace post content return 400 (Bad Request) when payload is not valid`(illegalContent: String?) {
    ...
    fun `should replace post content return 400 (Bad Request) when payload is valid and id is $le 0`(invalidId: Long) {
    ...
    @Test
    fun `should replace post content return 404 (Not Found) when payload is valid, id is $ge 1 and post does not exist`() {
        val expected = NOT_FOUND_ERROR.copy(
            message = "Post not found",
            path = VALID_REQUEST_PATH
        )

        every { replacePostContentUseCase(any()) } throws PostNotFoundException()
    ...
    companion object {
        private val VALID_REQUEST_PATH = "$BASE_PATH/${TEST_POST.id}/content"
        private val VALID_REQUEST = ContentUpdateRequest("New Content")
    }
}