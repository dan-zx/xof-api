...
class VoteOnPostEndpointTest : UserRestControllerBaseTest() {

    @Test
    fun `should vote on post return 204 (No Content) when userId is $ge 1, postId is $ge 1, payload is valid, user exists and post exists`() {
        every { voteOnPostUseCase(any()) } just runs
    ...
    fun `should vote on post return 400 (Bad Request) when userId is $le 0 or postId is $le 0`(invalidUserId: Long, invalidPostId: Long) {
        val requestPath = "$BASE_PATH/$invalidUserId/posts/$invalidPostId/votes"
        val expected = VALIDATION_ERROR.copy(
            path = requestPath,
            fieldErrors = mapOf(
                "userId" to "must be greater than or equal to 1",
                "postId" to "must be greater than or equal to 1"
            )
        )
    ...
    @Test
    fun `should vote on post return 400 (Bad Request) when userId is $ge 1, postId is $ge 1 and payload is not valid`() {
    ...
    fun `should vote on post return 404 (Not Found) when userId is $ge 1, postId is $ge 1, payload is valid and either user does not exist or post does not exist`(exceptionThrown: Exception, expectedMessage: String) {
        val expected = NOT_FOUND_ERROR.copy(
            message = expectedMessage,
            path = VALID_REQUEST_PATH
        )

        every { voteOnPostUseCase(any()) } throws exceptionThrown
    ...
    companion object ArgumentsProviders {
        ...
        private val VALID_REQUEST_PATH = "$BASE_PATH/${TEST_POST.user.id}/posts/${TEST_POST.id}/votes"
