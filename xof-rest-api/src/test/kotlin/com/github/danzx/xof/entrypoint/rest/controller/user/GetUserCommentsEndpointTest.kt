...
class GetUserCommentsEndpointTest : UserRestControllerBaseTest() {

    @Test
    fun `should get user comments return 200 (Ok) when no errors happen`() {
        val page = 1
        val pageSize = 10
    ...
    fun `should get user comments return 400 (Bad Request) when user id is invalid`(invalidId: Long) {
    ...
    fun `should get user comments return 400 (Bad Request) when pagination parameters are invalid`(invalidPage: Int, invalidPageSize: Int) {
        val expected = VALIDATION_ERROR.copy(
            path = VALID_REQUEST_PATH,
            fieldErrors = mapOf(
    ...
    companion object {
        private val VALID_REQUEST_PATH = "$BASE_PATH/${TEST_USER.id}/comments"
    }
}