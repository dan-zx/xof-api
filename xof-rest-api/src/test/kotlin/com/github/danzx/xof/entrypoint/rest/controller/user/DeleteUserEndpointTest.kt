...
class DeleteUserEndpointTest : UserRestControllerBaseTest() {

    @Test
    fun `should delete user return 204 (No Content) when id is $ge 1 and user exists`() {
        every { deleteUserByIdUseCase(any()) } just runs
    ...
    fun `should delete user return 400 (Bad Request) when id is $le 0`(invalidId: Long) {
    ...
    @Test
    fun `should delete user return 404 (Not Found) when id is $ge 1 and user does not exist`() {
        val expected = NOT_FOUND_ERROR.copy(
            message = "User not found",
            path = VALID_REQUEST_PATH
        )

        every { deleteUserByIdUseCase(any()) } throws UserNotFoundException()
